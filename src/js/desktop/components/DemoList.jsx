...
const Demo = React.createClass({
  getInitialState() {
    return {
      ...
      isPressed: false,
      lastPressed: 0,
      order: range(this.props.selectedVideos.length)
    };
  },
  ...
  handleMouseMove({pageY}) {
    ...
    if (isPressed) {
      ...
      const row = clamp(Math.round(mouse / 100), 0, this.props.selectedVideos.length - 1);
  ...
  render() {
    ...
    this.props.selectedVideos.forEach((elem, i) => {
      if (lastPressed === i && isPressed) {
        ...
        endValue.push( {
          ...
          y: {val: mouse, config: []},
          title: elem.title,
          selectedBy: elem.selectedVideos
        });
      }
      else {
        console.log(elem, i)
    ...
    return (
      <Spring endValue={endValue}>
        {items =>
          <div className="demo8 .video-queue-list">
            {items.map(({scale, shadow, y, title, selectedBy}, n) =>
              <div
                key={n}
                ...
                {order.indexOf(n) + 1 + title + selectedBy}
              </div>
            )}
          </div>
        }
